...
export default function CreateLineRentalAdSheet({
  visible,
  onClose,
  onSave,
  editingAd,
  ...
  useEffect(() => {
    if (visible) {
      if (editingAd) {
        ...
        setPercentSalon(
          editingAd.percentSalon
            ? toPersianDigit(String(editingAd.percentSalon))
            : '',
        );
        setPercentPartner(
          editingAd.percentPartner
            ? toPersianDigit(String(editingAd.percentPartner))
            : '',
        );
        setFixedAmount(
          editingAd.fixedAmount
            ? formatPriceInput(String(editingAd.fixedAmount))
            : '',
        );
        setFixedDeposit(
          editingAd.fixedDeposit
            ? formatPriceInput(String(editingAd.fixedDeposit))
            : '',
        );
        setHourlyRate(
          editingAd.hourlyRate
            ? formatPriceInput(String(editingAd.hourlyRate))
            : '',
        );
      } else {
        setTitle('');
  ...
  const handlePercentSalonChange = text => {
    ...
    if (salonNum > 0 && salonNum <= 100) {
      setPercentPartner(toPersianDigit(String(100 - salonNum)));
    } else {
      setPercentPartner('');
    }
    if (errors.price) setErrors(prev => ({ ...prev, price: '' }));
  ...
  const handlePercentPartnerChange = text => {
    ...
    if (partnerNum > 0 && partnerNum <= 100) {
      setPercentSalon(toPersianDigit(String(100 - partnerNum)));
    } else {
      setPercentSalon('');
    }
    if (errors.price) setErrors(prev => ({ ...prev, price: '' }));
  ...
  const handleSave = () => {
    ...
    if (collabType === 'percent') {
      ...
      if (!salonNum || !partnerNum) {
        ...
        priceDisplay = `${toPersianDigit(String(salonNum))}-${toPersianDigit(
          String(partnerNum),
        )}`;
      }
      ...
      if (!fixedNum) {
        ...
        if (depositNum > 0) {
          priceDisplay = `${toPersianDigit(
            fixedNum.toLocaleString('en-US'),
          )} + ${toPersianDigit(depositNum.toLocaleString('en-US'))} رهن`;
        } else {
          priceDisplay = `${toPersianDigit(
            fixedNum.toLocaleString('en-US'),
          )} تومان`;
      ...
      if (!hourlyNum) {
        ...
        priceDisplay = `${toPersianDigit(
          hourlyNum.toLocaleString('en-US'),
        )} / ساعت`;
      }
    }
      ...
      >
        ...
        >
          ...
          <Text style={[s.hintText, { color: colors.textSecondary }]}>
            با ثبت آگهی لاین، می‌توانید همکار متخصص جذب کنید و درآمد سالن خود را
            افزایش دهید.
          </Text>
        </Card>
        ...
        <View style={s.section}>
          ...
          <Input
            ...
            error={errors.title}
            rightIcon={
              <Icon name="title" size={20} color={colors.textSecondary} />
            }
          />
        </View>
        ...
        <View style={s.section}>
          ...
          {selectedService ? (
            <View
              style={[
                s.servicePreview,
                {
                  backgroundColor: selectedService.color + '12',
            ...
            >
              <View
                style={[
                  s.servicePreviewIconBox,
                  { backgroundColor: selectedService.color + '25' },
                ]}
              >
                <Icon
          ...
          {errors.serviceTypeId ? (
            <View style={s.errorRow}>
              ...
              <Text style={[s.errorText, { color: '#E53935' }]}>
                {errors.serviceTypeId}
              </Text>
            </View>
          ) : null}
              ...
              return (
                ...
                >
                  ...
                  <Text
                    style={[
                      s.collabLabel,
                      { color: isSelected ? type.color : colors.textMain },
                    ]}
                  >
                    {type.label}
                  </Text>
                  <Text
                    style={[s.collabHint, { color: colors.textSecondary }]}
                    numberOfLines={1}
                  >
                    {type.hint}
                  </Text>
                  {isSelected ? (
                    <View
                      style={[s.checkBadge, { backgroundColor: type.color }]}
                    >
        ...
        {collabType ? (
          <View style={s.section}>
            <View style={s.sectionHeader}>
              <View
                style={[s.sectionIconBox, { backgroundColor: '#4CAF5018' }]}
              >
              ...
              <Card variant="default" padding={14} radius={14}>
                ...
                <View style={s.percentRow}>
                  <View style={s.percentField}>
                    <Text
                      style={[s.percentFieldLabel, { color: colors.primary }]}
                    >
                      سهم سالن
                    </Text>
                    <View
                    ...
                    >
                      ...
                      />
                      <Text
                        style={[s.percentSign, { color: colors.textSecondary }]}
                      >
                        ٪
                      </Text>
                    </View>
                ...
                {parseNumber(percentSalon) > 0 ? (
                  <View
                    style={[
                      s.percentSummary,
                      {
                        backgroundColor: isPercentValid
                          ? '#4CAF5010'
                          : '#FF980010',
                  ...
                  >
                    ...
                    >
                      {isPercentValid
                        ? `✓ مجموع: ۱۰۰٪ (${toPersianDigit(
                            String(parseNumber(percentSalon)),
                          )}٪ سالن + ${toPersianDigit(
                            String(parseNumber(percentPartner)),
                          )}٪ همکار)`
                        : `مجموع فعلی: ${toPersianDigit(
                            String(percentSum),
                          )}٪ — باید ۱۰۰٪ باشد`}
                    </Text>
                  </View>
                ...
                <View
                  style={[
                    s.priceGuideBox,
                    { backgroundColor: '#9C27B008', borderColor: '#9C27B025' },
                  ]}
                >
                  <Icon name="lightbulb" size={14} color="#9C27B0" />
                  <Text
                    style={[s.priceGuideText, { color: colors.textSecondary }]}
                  >
                    مثال: سالن ۴۰٪ و همکار ۶۰٪ → از هر ۱۰۰,۰۰۰ تومان درآمد،
                    ۴۰,۰۰۰ تومان به سالن و ۶۰,۰۰۰ تومان به همکار می‌رسد
                  </Text>
                </View>
              </Card>
            ) : null}
            ...
            {collabType === 'fixed' ? (
              <Card variant="default" padding={14} radius={14}>
                <Text style={[s.priceHint, { color: colors.textSecondary }]}>
                  مبلغ ثابت ماهانه اجاره لاین را وارد کنید. مبلغ رهن اختیاری است
                  (می‌تواند صفر باشد).
                </Text>
                <Input
                  ...
                  rightIcon={
                    <Text
                      style={[s.currencyText, { color: colors.textSecondary }]}
                    >
                      تومان
                    </Text>
                  }
                ...
                {!!fixedAmount ? (
                  ...
                  >
                    ...
                    >
                      همکار ماهانه {fixedAmount} تومان به سالن پرداخت می‌کند و
                      ۱۰۰٪ درآمد خدمات متعلق به خودش است.
                      {parseNumber(fixedDeposit) > 0 ? (
                        <Text>{` همچنین ${fixedDeposit} تومان به عنوان رهن/ودیعه دریافت می‌کنید.`}</Text>
                      ) : null}
                    </Text>
                  </View>
                ) : null}
                ...
                {!!fixedAmount ? (
                  ...
                  >
                    <View style={s.combinedPreviewHeader}>
                      ...
                      <Text
                        style={[s.combinedPreviewTitle, { color: '#2196F3' }]}
                      >
            ...
            {collabType === 'hourly' ? (
              <Card variant="default" padding={14} radius={14}>
                <Text style={[s.priceHint, { color: colors.textSecondary }]}>
                  مبلغی که به ازای هر ساعت استفاده از لاین دریافت می‌کنید را
                  وارد کنید
                </Text>
                <Input
                ...
                {hourlyNum > 0 ? (
                  ...
                  >
                    <View style={s.combinedPreviewHeader}>
                      ...
                      <Text
                        style={[s.combinedPreviewTitle, { color: '#FF9800' }]}
                      >
                        پیش‌نمایش محاسبه
                      </Text>
                    </View>
                    <View style={s.combinedPreviewRow}>
                      ...
                      >
                        {toPersianDigit(
                          (hourlyNum * 2).toLocaleString('en-US'),
                        )}{' '}
                        تومان
                      </Text>
                    </View>
                    <View style={s.combinedPreviewRow}>
                      ...
                      >
                        {toPersianDigit(
                          (hourlyNum * 4).toLocaleString('en-US'),
                        )}{' '}
                        تومان
                      </Text>
                    </View>
                    <View style={s.combinedPreviewRow}>
                      ...
                      >
                        {toPersianDigit(
                          (hourlyNum * 8).toLocaleString('en-US'),
                        )}{' '}
                        تومان
                      </Text>
                    </View>
                ...
                <View
                  style={[
                    s.priceGuideBox,
                    { backgroundColor: '#FF980008', borderColor: '#FF980025' },
                  ]}
                >
                  ...
                  <Text
                    style={[s.priceGuideText, { color: colors.textSecondary }]}
                  >
                    مدل ساعتی برای لاین‌هایی که به صورت موقت یا چند ساعت در روز
                    استفاده می‌شوند مناسب است. همکار فقط به ازای ساعاتی که از
                    لاین استفاده می‌کند پرداخت می‌کند.
                  </Text>
                </View>
              </Card>
            ) : null}
          ...
          >
            {lineImage ? (
              <View style={s.pickedImageWrap}>
                ...
                <View
                  style={[s.editBadge, { backgroundColor: colors.primary }]}
                >
              ...
              <View style={s.imagePlaceholder}>
                ...
                <Text
                  style={[s.placeholderHint, { color: colors.textSecondary }]}
                >
